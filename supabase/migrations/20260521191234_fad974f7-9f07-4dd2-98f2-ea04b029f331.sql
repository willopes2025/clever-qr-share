CREATE OR REPLACE FUNCTION public.get_whatsapp_message_stats(
  p_start timestamptz,
  p_end timestamptz
)
...
AS $$
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE im.status IN ('delivered', 'read'))::bigint AS delivered,
...
GRANT EXECUTE ON FUNCTION public.get_whatsapp_message_stats(timestamptz, timestamptz) TO authenticated;
