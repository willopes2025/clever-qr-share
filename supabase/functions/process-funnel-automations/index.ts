...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { dealId, fromStageId, toStageId, triggerType, messageContent, tagName, customFieldKey, customFieldValue, isNewDeal, oldResponsible, newResponsible, skipDelay } = payload;
          ...
          case 'move_stage': {
            ...
            if (targetStageId) {
              ...
              try {
                ...
                console.log(`[FUNNEL-AUTOMATIONS] Triggered on_stage_enter for stage: ${targetStageId}`);
              } catch (err) {
                console.error(`[FUNNEL-AUTOMATIONS] Error triggering on_stage_enter:`, err);
              }
            }
            results.push({ automationId: automation.id, success: true });
            break;
...
Retorne APENAS a mensagem, sem explicações ou aspas.`;
          ...
          case 'generate_asaas_pix': {
            console.log(`[FUNNEL-AUTOMATIONS] Generating Asaas PIX charge for deal ${dealId}`);
            ...
            if (!chargeValue || chargeValue <= 0) {
              ...
              results.push({ automationId: automation.id, success: false, error: `Valor inválido no campo ${valueField}` });
              break;
            }
            ...
            if (!contactPhone) {
              console.error(`[FUNNEL-AUTOMATIONS] No phone for contact`);
              results.push({ automationId: automation.id, success: false, error: 'Contato sem telefone' });
              break;
            }
            ...
            if (!asaasIntegration) {
              ...
              if (teamMember?.organization_id) {
                const { data: org } = await supabase
                  ...
                  .single();
                if (org?.owner_id) ownerId = org.owner_id;
              }
              ...
              if (!ownerIntegration) {
                console.error(`[FUNNEL-AUTOMATIONS] No Asaas integration found`);
                results.push({ automationId: automation.id, success: false, error: 'Integração Asaas não configurada' });
                break;
              }
              
              // Use owner integration
              Object.assign(asaasIntegration || {}, ownerIntegration);
            }
            
            const asaasCreds = (asaasIntegration?.credentials || {}) as Record<string, string>;
            const asaasToken = asaasCreds.access_token;
            const asaasEnv = asaasCreds.environment || 'production';
            const asaasBaseUrl = asaasEnv === 'sandbox' ? 'https://api-sandbox.asaas.com/v3' : 'https://api.asaas.com/v3';
            
            if (!asaasToken) {
              results.push({ automationId: automation.id, success: false, error: 'API key Asaas não configurada' });
              break;
            }
            ...
            if (!asaasCustomerId) {
              // Search by phone
              const searchRes = await fetch(`${asaasBaseUrl}/customers?cpfCnpj=&mobilePhone=${cleanPhone}&limit=1`, {
            ...
            if (!asaasCustomerId) {
              ...
              const createRes = await fetch(`${asaasBaseUrl}/customers`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'access_token': asaasToken },
                body: JSON.stringify(customerPayload),
              });
              const createData = await createRes.json();
              
              if (!createRes.ok) {
                console.error(`[FUNNEL-AUTOMATIONS] Failed to create Asaas customer:`, createData);
                results.push({ automationId: automation.id, success: false, error: `Erro ao criar cliente: ${createData.errors?.[0]?.description || 'unknown'}` });
                break;
              }
              
              asaasCustomerId = createData.id;
              console.log(`[FUNNEL-AUTOMATIONS] Created Asaas customer: ${asaasCustomerId}`);
              
              // Save asaas_customer_id on contact
              if (deal.contact_id) {
                await supabase.from('contacts').update({ asaas_customer_id: asaasCustomerId }).eq('id', deal.contact_id);
              }
            } else if (groupName) {
              // Update existing customer with groupName
            ...
            console.log(`[FUNNEL-AUTOMATIONS] Creating PIX payment:`, paymentPayload);
            ...
            if (!payRes.ok) {
              console.error(`[FUNNEL-AUTOMATIONS] Failed to create payment:`, payData);
              results.push({ automationId: automation.id, success: false, error: `Erro ao criar cobrança: ${payData.errors?.[0]?.description || 'unknown'}` });
              break;
            }
            
            console.log(`[FUNNEL-AUTOMATIONS] PIX payment created: ${payData.id}, invoiceUrl: ${payData.invoiceUrl}`);
            results.push({ automationId: automation.id, success: true, paymentId: payData.id });
            break;
          }
