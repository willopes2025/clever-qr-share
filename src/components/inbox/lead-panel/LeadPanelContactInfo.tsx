...
import { toBrazilTime, formatDateOnly, formatDateTimeShort } from "@/lib/date-utils";
  ...
  return (
    <div className="px-3 py-2 border-t border-border/30 space-y-2">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>Primeira conversa: {formatDateOnly(conversation.created_at)}</span>
      </div>
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>
          Última mensagem: {conversation.last_message_at 
            ? formatDateTimeShort(conversation.last_message_at)
            : "Nenhuma"}
        </span>
      </div>
    </div>
